...
use arduino_hal::{
    hal::port::Dynamic,
    port::{mode::{Output, PullUp, Input}, Pin},
};
use panic_halt as _;
mod millis;
...
fn main() -> ! {
    ...
    const GREEN_RED_TIME: i8 = 1;
    const YELLOW_TIME: i8 = 5;
    ...
    fn ms_change(ms_thing:i16,btn:&mut Pin<Input<PullUp>,Dynamic>)->i16{
        return if ms_thing==1000 {lightbutton(btn)} else {ms_thing};
    }
    ...
    loop {
        match light_state {
            ...
            YELLOW_BEFORE_GREEN | YELLOW_BEFORE_RED => {
